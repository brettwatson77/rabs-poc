...
const MasterSchedule = () => {
  ...
  /* ------------------------------------------------------------------
   * Participants list and selection for the Create-Program modal
   * ------------------------------------------------------------------ */
  const [participantsList, setParticipantsList] = useState([]);
  const [selectedParticipants, setSelectedParticipants] = useState([]);
  const [participantSearchTerm, setParticipantSearchTerm] = useState('');
  // Venues list for venue dropdown in create-program modal
  const [venuesList, setVenuesList] = useState([]);
  // Inline venue creation state
  const [showCreateNewVenue, setShowCreateNewVenue] = useState(false);
  ...
  const handleAddParticipant = (participantId) => {
    ...
    if (selectedParticipants.some(p => p.id === participantId)) return;
    
    // Add to selected list
    setSelectedParticipants([...selectedParticipants, participant]);
    
    // Initialize billing data for this participant
    if (!billingData[participantId]) {
      setBillingData({
        ...billingData,
        [participantId]: { hours: '', codes: [{ code: '', hours: '' }] }
      });
    }
  };
  ...
  // Handle creating a new venue
  const handleCreateNewVenue = async () => {
    // Validate form
    if (!newVenueForm.name || !newVenueForm.address) {
    ...
    try {
      ...
      const response = await axios.post('/api/v1/venues', {
        name: newVenueForm.name,
        address: newVenueForm.address,
        capacity: newVenueForm.capacity ? parseInt(newVenueForm.capacity) : null,
        wheelchair_accessible: newVenueForm.wheelchair_accessible
      });
      ...
      alert('Failed to create venue: ' + (err.message || 'Unknown error'));
    } finally {
  ...
  // Fetch all venues once when component mounts
  useEffect(() => {
  ...
  const filteredParticipants = participantsList.filter(participant => {
    ...
    return fullName.includes(searchLower) || 
           (participant.ndis_number && participant.ndis_number.toLowerCase().includes(searchLower));
  });
  ...
  // Handle create new master card - now creates a program directly
  const handleCreateMasterCard = async (formData) => {
    try {
      // Set up days of week array based on selected day
      const selectedDayOfWeek = new Date(formData.startDate).getDay();
      const daysOfWeek = formData.isRepeating 
        ? (formData.daysOfWeek || [selectedDayOfWeek]) 
        : [selectedDayOfWeek];
      
      // Create program data structure for direct program creation
      const programData = {
        name: formData.name,
        program_type: formData.type,
        start_date: formData.startDate,
        // End-date logic:
        // • Repeating programs: use repeatEnd if provided, otherwise NULL (repeat forever)
        // • One-off programs   : end_date must equal start_date so the loom can
        //   regenerate the single instance when the window expands/contracts.
        end_date: formData.isRepeating
          ? (formData.repeatEnd || null)
          : formData.startDate,
        repeat_pattern: formData.isRepeating ? formData.repeatPattern : 'none',
        ...
        time_slots: formData.timeSlots.map(slot => ({
          ...
          label: slot.type === 'other' ? slot.description : 
                 slot.type === 'pickups' ? 'Pick-up Run' : 
                 slot.type === 'dropoffs' ? 'Drop-off Run' : 
                 slot.type === 'meal' ? 'Meal Break' : 'Activity',
          type: slot.type
        })),
        notes: formData.notes,
        staff_assignment_mode: formData.staffAssignment,
        additional_staff_count: 0, // Placeholder for future implementation
        participants: formData.participants.map(p => ({
          id: p.id,
          billing: billingData[p.id] || { hours: '', codes: [{ code: '', hours: '' }] }
        }))
      };
      
      console.log('Creating program:', programData);
      
      // Send to new direct program creation endpoint
      ...
      if (response.data && response.data.success) {
        ...
        // Refresh schedule data to show new program
        fetchScheduleData();
      } else {
      ...
      const errorMessage = err.response?.data?.errors 
        ? `Failed to create program: ${err.response.data.errors.join(', ')}` 
        : err.message || 'Failed to create program. Please try again.';
  ...
  const getCardsForDay = (day) => {
    ...
    return scheduleData.filter(card => {
      ...
      // Exact match for simple YYYY-MM-DD strings
      if (card.date === dateStr) return true;

      /* 
       * Some APIs return full ISO strings
       * (e.g. 2025-08-06T14:00:00.000Z).  Use
       * partial matching so the card is still
       * included in the correct day column.
       */
      if (typeof card.date === 'string' && card.date.includes(dateStr)) {
        return true;
      }
      ...
      {showCreateModal && selectedDay && (
        <Modal onClose={() => setShowCreateModal(false)} className="advanced-program-modal">
          ...
          <div className="program-modal-body">
                ...
                <>
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setProgramForm({ ...programForm, venue_id: e.target.value })
                        }
                      >
                  ...
                  <div className="venue-options">
                    <label className="checkbox-label">
                      <input
                        type="checkbox"
                        checked={showCreateNewVenue}
                        onChange={() => setShowCreateNewVenue(!showCreateNewVenue)}
                      />
                      Create new venue
                    </label>
                  </div>
                  ...
                  <div className="new-venue-form">
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewVenueForm({ ...newVenueForm, name: e.target.value })
                          }
                          placeholder="e.g., Community Center, Bowling Alley"
                          required
                        />
                      </div>
                    </div>
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewVenueForm({ ...newVenueForm, address: e.target.value })
                          }
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewVenueForm({ ...newVenueForm, capacity: e.target.value })
                          }
                      ...
                      <div className="form-group">
                        <label className="checkbox-label">
                          <input
                            type="checkbox"
                            checked={newVenueForm.wheelchair_accessible}
                            onChange={(e) =>
                              setNewVenueForm({ ...newVenueForm, wheelchair_accessible: e.target.checked })
                            }
                          />
                    ...
                    <div className="venue-form-actions">
                      ...
                      <button
                        ...
                        disabled={creatingVenue || !newVenueForm.name || !newVenueForm.address}
                      >
            ...
            {/* Participant Selection - IMPROVED DROPDOWN VERSION */}
            <div className="program-section">
              <h3>Participant Selection</h3>
              <div className="participant-selector">
                {/* Search and dropdown */}
                <div className="participant-dropdown-container">
                  ...
                  >
                    ...
                    {filteredParticipants.map(participant => (
                      ...
                      >
                        {participant.first_name} {participant.last_name} 
                        {participant.supervision_multiplier > 1 ? ` (${participant.supervision_multiplier}x)` : ''}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="selected-participants">
                  ...
                  {/* ------------------------------------------------------------------
                     Billing Help Note
                     ------------------------------------------------------------------
                     Clarify how the dual-hours system works so staff understand that the
                     “Total Hours” field drives billing by default, but can be overridden
                     on a per-code basis.  If individual code hours are blank, the total
                     hours value will be apportioned evenly across all billing codes.
                  ------------------------------------------------------------------ */}
                  <div className="billing-help-note">
                    <small>
                      <strong>Hours System:</strong> Enter total hours for each participant
                      above, then specify billing codes below.&nbsp;If code-specific hours are
                      left blank, the total hours will be automatically distributed across
                      all listed billing codes.
                    </small>
                  </div>
                  
                  {selectedParticipants.length === 0 ? (
                    <div className="no-participants">No participants selected yet</div>
                  ) : (
                    <div className="participant-cards">
                      {selectedParticipants.map(participant => {
                        const participantId = participant.id;
                        return (
                          <div key={participantId} className="participant-card full-width">
                            <div className="participant-header">
                              <div className="participant-name-section">
                                ...
                                {participant.supervision_multiplier > 1 && (
                                  <span className={`supervision-tag multiplier-${Math.floor(participant.supervision_multiplier * 4)}`}>
                                    {participant.supervision_multiplier}x
                                  </span>
                                )}
                              </div>
                            ...
                            <div className="participant-billing">
                              <div className="billing-hours">
                                ...
                                <input
                                  ...
                                  onChange={(e) => updateParticipantBilling(
                                    participantId, 
                                    'hours', 
                                    e.target.value
                                  )}
                                ...
                                {(billingData[participantId]?.codes || [{ code: '', hours: '' }]).map(
                                  (codeEntry, codeIdx) => (
                                    <div key={codeIdx} className="billing-code-row">
                                      ...
                                      >
                                        <option value="">Select NDIS code...</option>
                                      ...
                                      <div className="code-hours">
                                        ...
                                        <div className="code-buttons">
                                          <button
                                            ...
                                            title="Add another billing code"
                                          >
                                            ➕
                                          </button>
